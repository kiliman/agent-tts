...
export class ConfigLoader extends EventEmitter {
  ...
  private async loadConfigFile(filePath: string): Promise<AgentTTSConfig | null> {
    try {
      ...
      const result = configFunction(moduleExports, requireFunc, moduleObj, filePath, path.dirname(filePath))
  ...
  private isConfigDependency(filePath: string): boolean {
    ...
    return filePath.startsWith(this.configDir) && (filePath.endsWith('.js') || filePath.endsWith('.ts'))
  }
