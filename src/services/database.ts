...
export class DatabaseManager {
  ...
  private initializeTables(): void {
    ...
    try {
      ...
      const tableInfo = this.db
        .prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='tts_queue'")
        .get() as any
  ...
  updateFileState(state: FileState): void {
    this.db
      ...
      .run(state.filepath, state.lastModified, state.fileSize, state.lastProcessedOffset)
  }
  ...
  getEntryById(id: number): TTSQueueEntry | null {
    const row = this.db.prepare(`SELECT * FROM tts_queue WHERE id = ?`).get(id) as any
  ...
  toggleFavorite(id: number): boolean {
    ...
    const current = this.db.prepare('SELECT is_favorite FROM tts_queue WHERE id = ?').get(id) as any
    ...
    this.db.prepare('UPDATE tts_queue SET is_favorite = ? WHERE id = ?').run(newValue, id)
  ...
  getFavoritesCount(profile?: string): number {
    if (profile) {
      const result = this.db
        .prepare('SELECT COUNT(*) as count FROM tts_queue WHERE profile = ? AND is_favorite = 1')
        .get(profile) as any
      return result?.count || 0
    } else {
      const result = this.db.prepare('SELECT COUNT(*) as count FROM tts_queue WHERE is_favorite = 1').get() as any
      return result?.count || 0
    }
  }
