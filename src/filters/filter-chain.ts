...
export class FilterChain {
    ...
    for (const config of filterConfigs) {
      if (config.filter) {
        ...
        if (existingFilter) {
          ...
          const builtInFilter = this.createBuiltInFilter(config.name, config.enabled ?? true, config.options)
          if (builtInFilter) {
  ...
  private createBuiltInFilter(name: string, enabled: boolean, options?: any): BaseFilter | null {
    switch (name) {
      case 'pronunciation': {
