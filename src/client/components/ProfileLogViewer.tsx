...
export function ProfileLogViewer({
  ...
  useEffect(() => {
    ...
    const handleStatusChanged = (data: any) => {
      ...
      if (data.playing && data.playingId) {
        ...
        setLogs((prevLogs) => prevLogs.map((log) => (log.id === data.playingId ? { ...log, status: 'playing' } : log)))
      } else if (!data.playing && data.playedId) {
        setPlayingId(null)
        // Update the log entry status to 'played'
        setLogs((prevLogs) => prevLogs.map((log) => (log.id === data.playedId ? { ...log, status: 'played' } : log)))
      }
    }
  ...
  const loadLogs = async (reset: boolean = false) => {
    ...
    try {
      ...
      console.log(
        `Loading ${favoritesOnly ? 'favorite' : 'all'} logs for profile: ${profile}, offset: ${currentOffset}`,
      )
      const response = await apiClient.getLogs(50, profile, favoritesOnly, currentOffset, cwdFilter)
  ...
  const handleToggleFavorite = async (id: number) => {
    try {
      ...
      if (response.success) {
        ...
        setLogs((prevLogs) =>
          prevLogs.map((log) => (log.id === id ? { ...log, isFavorite: response.isFavorite } : log)),
        )
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 sm:gap-4 flex-1 min-w-0">
              {profileInfo.avatarUrl || profileInfo.profileUrl ? (
                <img
                  src={getResourceUrl(profileInfo.avatarUrl || profileInfo.profileUrl)}
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {favoritesCount > 0 && (
                    ...
                    >
                      ...
                      <span className="text-sm text-gray-600 dark:text-gray-400 sm:hidden">{favoritesCount}</span>
                    </button>
                  )}
                </div>
                {(profileInfo.parserName || profileInfo.model) && (
                  <div className="hidden sm:flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mt-1">
                    ...
                    {profileInfo.modelIconUrl && (
                      <img src={getResourceUrl(profileInfo.modelIconUrl)} alt={profileInfo.model} className="w-4 h-4" />
                    )}
